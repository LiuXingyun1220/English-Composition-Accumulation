...
### 求合同矩阵的方法
目标：求一个可逆矩阵 $C$，使得 $C^T A C = \Lambda$（$\Lambda$ 的对角元不一定是特征值，通常是 $1, -1, 0$）。

适用场景：题目只说“化为标准形”或“规范形”，没提正交。此时用以下方法比求特征值快得多。

#### 方法 A：初等变换法（推荐，直接出结果）

利用分块矩阵 $\begin{pmatrix} A \\ E \end{pmatrix}$ 进行操作。

1. **构造矩阵**：写出 $\begin{pmatrix} A \\ E \end{pmatrix}$。
    
2. **操作规则**：
    
    - 对 $A$ 做一次**行变换**（$r_i + k r_j$），必须紧接着对 $A$ 做一次相同的**列变换**（$c_i + k c_j$）。目的是把 $A$ 化为对角阵 $\Lambda$。
        
    - 对下方的 $E$，**只做列变换**（$c_i + k c_j$），**跟着 $A$ 的列变换走**。
        
3. 结果：
    
    当 $A$ 变成对角阵 $\Lambda$ 时，下方的 $E$ 就自动变成了所求的 合同矩阵 $C$。
    
    $$\begin{pmatrix} A \\ E \end{pmatrix} \xrightarrow{\text{合同变换}} \begin{pmatrix} \Lambda \\ C \end{pmatrix}$$
...
2. **配方**：通过代数配方（凑完全平方），将 $f(x)$ 化为 $d_1 y_1^2 + d_2 y_2^2 + \dots$。
    
3. 得到关系式：
    
    配方过程中，你会得到新坐标 $y$ 和旧坐标 $x$ 的关系，通常形式是：
    
    $$\begin{cases} y_1 = x_1 + \dots \\ y_2 = x_2 + \dots \\ \vdots \end{cases} \quad \Rightarrow \quad y = Px$$
    
4. ⚠️ 致命陷阱（求逆）：
    
    题目求的是合同变换矩阵 $C$，指的是 $x = Cy$。
